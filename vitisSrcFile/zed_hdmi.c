...
#define ADV7511_ADDR   0x72   // (0x72>>1) == 0x39
...
void HDMI_config(zed_iic_t *piic)
{
	...
	for (i = 0; i < CARRIER_HDMI_OUT_CONFIG_LEN; i++)
	{
		...
		num_bytes = piic->fpIicWrite(piic, carrier_hdmi_out_config[i][0], carrier_hdmi_out_config[i][1], &(carrier_hdmi_out_config[i][2]), 1);
	}
}
...
int check_hdmi_hpd_status(zed_iic_t *piic, u8 Address)
{
	...
	piic->fpIicRead(piic, Address,ADV7511_HDP_REG_ADDR,&data,1);
...
void configure_adv7511(zed_iic_t *piic, u8 Address)
{
	...
	piic->fpIicWrite(piic, Address, 0x41,0x10, 1);


	// Fixed registers that must be set on power up
	piic->fpIicWrite(piic, Address, 0x98, 0x03, 1);
	piic->fpIicWrite(piic, Address, 0x9A, 0xE0, 1);
	piic->fpIicWrite(piic, Address, 0x9C, 0x30, 1);
	piic->fpIicWrite(piic, Address, 0x9D, 0x61, 1);
	piic->fpIicWrite(piic, Address, 0xA2, 0xA4, 1);
	piic->fpIicWrite(piic, Address, 0xA3, 0xA4, 1);
	piic->fpIicWrite(piic, Address, 0xE0, 0xD0, 1);
	piic->fpIicWrite(piic, Address, 0xF9, 0x00, 1);

	// Color Space Conversion
	piic->fpIicWrite(piic, Address, 0x18, 0xE7, 1); //CSC Enabled (YCbCr to RGB)

	// Set RGB in AVinfo Frame
	piic->fpIicWrite(piic, Address, 0x55, 0x00, 1);
	// Aspect Ration
	piic->fpIicWrite(piic, Address, 0x56, 0x28, 1);

	// HPD Control always high
	piic->fpIicWrite(piic, Address, 0xD6, 0xC0, 1);

	// DVI Mode, no HDCP
	piic->fpIicWrite(piic, Address, 0xAF, 0x04, 1);
